...
import { getProductsByStoreId, getReservations } from '@/lib/github'
...
export default function StoreStats({ user, store }: StoreStatsProps) {
  ...
  const loadStats = async () => {
    try {
      const [products, allReservations] = await Promise.all([
        getProductsByStoreId(store.id),
        getReservations(),
      ])

      const storeReservations = allReservations.filter(r => r.storeId === store.id)
      const activeProducts = products.filter(p => p.status === 'available')
      
      const completedReservations = storeReservations.filter(r => r.status === 'completed')
      ...
      setStats({
        ...
        totalReservations: storeReservations.length,
        activeReservations: storeReservations.filter(r => r.status === 'active').length,
        completedReservations: completedReservations.length,
        cancelledReservations: storeReservations.filter(r => r.status === 'cancelled').length,
